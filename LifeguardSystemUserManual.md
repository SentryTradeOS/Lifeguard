...
**Lifeguard** is a lightweight, enterprise-grade background monitoring system designed specifically for highly memory-constrained VPS environments.

Its primary task is to run in the background, monitoring your **MT4 Trading Terminals** and **HaNodeClient** 24/7. When it detects a crash, freeze, or disconnection in the target programs, Lifeguard automatically kills the zombie processes, executes a restart, and sends an alert via Telegram.
...
* **Seamless Hot Reload:** Changes to the configuration file (config.json) take effect immediately upon saving without needing to restart Lifeguard.  
...
| :---- | :---- | :---- |
| NodeId | Unique identifier for this VPS/Server. Displayed in Telegram alerts to pinpoint the source. If missing or undefined, a fail-safe default of "LifeguardNode" is used. | Custom name (e.g., WinNode-A) |
| MT4Instances | List of MT4 instances to monitor. Format: "Custom\_ID" : "Absolute\_Path\_to\_exe". | Set according to environment |
| HaNodeClient.ExePath | Absolute path to the HaNodeClient executable. Leave blank "" if monitoring is not required. | Set according to environment |
...
| AlertReminderIntervalMs | **Persistent Crash Reminder Interval**. How often to send a Telegram reminder when a software is completely dead. | 3600000 (1 hour) |
...
In the EA's OnTimer (e.g., every 3 seconds), execute SetEvent to wake Lifeguard.
...
When Lifeguard is running, the window title bar dynamically displays the current highest-priority system status:
...
**Q1: Why did the terminal show a yellow HOT RELOAD message after modifying config.json, but then immediately restarted my MT4?**
